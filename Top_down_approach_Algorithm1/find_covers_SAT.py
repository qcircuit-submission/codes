...
def find_common_covers_result(F, n, maxd, ks, M, prefix):
    ...
    ai = 0
    XC_star = M[0][:]
    XC_str  = ["1"] + [f"x{i}" for i in range(n)] + [f"C{i}" for i in range(sum(ks))]
    for d in range(maxd):
        n3 = len(XC_star)
        result.append(f"########## AND-depth {d} to {d + 1} ##########")
        for i in range(ks[d]):
            ...
            result.append(f"C{sum(ks[:d])+i} = ({' + '.join(Di1_terms)})*({' + '.join(Di2_terms)})")
    ...
    for i in range(len(F)):
        ...
        result.append(f"f{i} = {' + '.join(f_terms)}\n   = {f}")
