...
def linears_to_matrix(L: list, cols: int):
    rows = len(L)
...
def check_two_states_qubits(states: list, dim: int):
    states_n = len(states)
    ...
    for i in range(states_n): 
        rank1 = linears_to_matrix(L=nonzero_states, cols=dim).rank()
        # rank = r
        if rank1 == dim:
            break 
        ...
        rank2 = linears_to_matrix(L=nonzero_states, cols=dim).rank()
...
def select_step(initial_states: list, candidate_states: list, dim: int, target_n: int):
    ...
    for i in range(states_n):
        rank1 = linears_to_matrix(L=selected_states, cols=dim).rank()
        # rank = dim
        if rank1 == dim:
            break 
        ...
        rank2 = linears_to_matrix(L=selected_states, cols=dim).rank()
    ...
    # print(f"final size: {len(selected_states)}")
    return selected_states

# transform from S_pre to S
# (1) size(S_pre) = size(S)
# (2) rank(S_pre) = rank(S) = dim(number of columns)
def get_state_transform_matrix(S_pre, S, dim):
    rows = len(S_pre)

    # Si_pre -> A, Si -> B, find an invertible matrix MT
    # such that MT * A = B
    A = linears_to_matrix(L=S_pre, cols=dim)
    B = linears_to_matrix(L=S, cols=dim)
...
def output_part(n: int, m: int, cur_dim: int, cur_states: list, Bs: list, By: list):
    ...
    # AND-layer-s
    ns = len(Bs) // 2
    a = 2*ns - linears_to_matrix(Bs, cols=dim).rank()

    nonzero_qubits, zero_qubits = check_two_states_qubits(states=states, dim=dim)
    zero_states    = [states[i] for i in zero_qubits]
    nonzero_states = [states[i] for i in nonzero_qubits]
    zero_states_n = len(zero_states)

    # additionally consider outputs qubits(must m newly allocated)
    allocate_qubits_n = m
    ...
    # Generate the outputs Y (need store in the last allocated qubits)
    outputs_qubits = qubits[-m:]
    ny = len(By) // 2
    ay = 2*ny - linears_to_matrix(By, cols=dim).rank()
    ...
    Y_states = select_step(initial_states=By, candidate_states=Y_pre_states
                            , dim=dim, target_n=CNOT_n)
   
    MT_Y = get_state_transform_matrix(S_pre=Y_pre_states, S=Y_states, dim=dim)
    CNOT_layers_qubits.append(CNOT_qubits)
...
def classical_to_quantum(n: int, m: int, s: int, C: list):
    ...
    for i in range(1, s):
        ...
        ai = 2*ni - linears_to_matrix(B[i], cols=dim).rank()
